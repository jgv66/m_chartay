...
let getUsuarios = async function() {
    ...
    return new Promise((resolve, reject) => {
        ...
        return sqlpool[1] /* la primera empresa tiene a todos los usuarios */
            ...
            .then(resultado => {
                console.log('Usuarios encontrados: ', resultado.recordset.length);
...
let getEmpresa = (rut) => {
    //
    return new Promise((resolve, reject) => {
        // console.log(5555, rut, rut.upper());
        var emp;
        usuarios.forEach(element => {
            if (element.rut === rut.toUpperCase()) {
                emp = element;
            }
        });
        console.log('buscando rut', rut, 'emp', emp);
        if (emp === undefined) {
            reject(0);
        } else {
...
let registraActividad = (empresa, rut, actividad) => {
    ...
    //
    fs.appendFile(FACTURACION, reg, (err) => {});
    //
};
        ...
        reg.getBase64Cert(sqlpool, req.body)
            .then(function(data) {
                ...
                contruyeHTML(data.datos, fullpathHTM, req.body.empresa)
                    .then(htmlContent => {
                        // console.log('HTML creado ', fullpathPDF);
                        try {
                            wkhtmltopdf(
                                htmlContent, {
                                    output: fullpathPDF,
...
enviarCorreoVacaciones = function(req, response, data) {
    ...
    transporter.sendMail(mailOptions, function(error, info) {
        if (error) {
            console.log('error en sendmail->', error);
            response.status(500).json({ resultado: 'error', mensaje: error.message });
        } else {
            //
